...
export async function syncArticleUnits(tx: Tx, articleId: string, units: ArticleUnitInput[]) {
  ...
  if (incomingIds.length > 0) {
    const existingCount = await tx.productPackagingUnit.count({
  ...
  await tx.productPackagingUnit.updateMany({
    where: {
      articleId,
  ...
  for (const unit of normalizedUnits) {
    const saved = unit.id
      ? await tx.productPackagingUnit.update({
          where: { id: unit.id },
          data: {
            label: unit.label,
      ...
      : await tx.productPackagingUnit.create({
          data: {
            articleId,
            label: unit.label,
            quantity: unit.quantity,
